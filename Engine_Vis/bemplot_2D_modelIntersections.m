function [intersectionTraces] = bemplot_2D_modelIntersections(model, obsPlane, options)
    
    numTissues = length(model.tissue);
    
    if nargin < 3 || isempty(options) || ~isfield(options.color) || isempty(options.color)
