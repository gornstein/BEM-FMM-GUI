...
Z       = 55.5;              %   XY Cross-section position, mm
...
for m = 1:m_max
    tS{m} = t(Indicator(:,1) == m, :);
    nS{m} = normals(Indicator(:,1) == m, :);
...
%%  Head graphics
tissue_to_plot = 'GM';
t0 = t(Indicator(:,1)==find(strcmp(tissue, tissue_to_plot)), :);    % (change indicator if necessary: 1-skin, 2-skull, etc.)
...
%%  Plot coil centerline
hold on;
plot(1e3*pointsline(:, 1), 1e3*pointsline(:, 2), '-r', 'lineWidth', 2);
...
%%  Plot coil centerline
hold on;
plot(1e3*pointsline(:, 1), 1e3*pointsline(:, 3), '-r', 'lineWidth', 2);
...
%%  Plot coil centerline
hold on;
plot(1e3*pointsline(:, 2), 1e3*pointsline(:, 3), '-r', 'lineWidth', 2);
